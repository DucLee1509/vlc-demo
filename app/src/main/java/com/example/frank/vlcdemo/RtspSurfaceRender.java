...
public class RtspSurfaceRender implements GLSurfaceView.Renderer, RtspHelper.RtspCallback {
    ...
    public void onSurfaceChanged(GL10 gl, int width, int height) {
        ...
        RtspHelper.getInstance().setCallback(this); // Set the callback
    ...
    private void reconnect() {
        mGLSurfaceView.queueEvent(new Runnable() {
            ...
            public void run() {
                RtspHelper.getInstance().releasePlayer();
                RtspHelper.getInstance().createPlayer(mRtspUrl, mGLSurfaceView.getWidth(), mGLSurfaceView.getHeight(), RtspSurfaceRender.this);
            }
        });
    }

    @Override
    public void onRtspConnectionLost() {
        LogUtils.v("RTSP connection lost, attempting to reconnect...");
        reconnect();
    }
}
